...
from logic import A, conjoin, disjoin
...
def sentence1() -> Expr:
    ...
    instance2 = not_a % (not_b | C)
    
    instance3 = disjoin([not_a ,not_b, C]) # Cuando es más que un or usar disjoin

    return conjoin([instance1, instance2, instance3])
...
def sentence2() -> Expr:
    ...
    instance1 = C % (B | D)
    instance2 = A >> (not_b & not_d)
    instance3 = ~(B & not_c) >> A
    instance4 = not_d >> C

    return conjoin([instance1, instance2, instance3, instance4])
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PA_0 = logic.PropSymbolExpr("PacmanAlive", 0)
    PB_0 = logic.PropSymbolExpr("PacmanBorn", 0)
    PK_0 = logic.PropSymbolExpr("PacmanKilled", 0)
    PA_1 = logic.PropSymbolExpr('PacmanAlive', 1)

    instance1 = PA_1 % ((PA_0 & ~PK_0) | (~PA_0 & PB_0))
    instance2 = ~(PA_0 & PB_0)
    instance3 = PB_0

    pacmanAlive_0 = conjoin([instance1, instance2, instance3])
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    for key in model:
        list_keys = list_keys + [model[key]]

    if True in list_keys:
        return False
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for literal in literals:
        ...
        # and add it to the list to be conjoined. Discard literals already seen
        for inner_literal in literals:
            if literal != inner_literal and inner_literal not in seen:
                disjunction = disjoin([~literal, ~inner_literal])
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    expr_leastone = atLeastOne(literals)
    expr_mostone = atMostOne(literals)

    return conjoin([expr_leastone, expr_mostone])
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    Given:
        ...
        non_outer_wall_coords: list of (x, y) coordinates of the entire problem,
            ...
            possibly be in) #Lista de cuadrados de donde puede estar el pacman#
        ...
        sensorModel(t, non_outer_wall_coords) -> Expr: function that generates
            ...
            #No se implementa porque sabemos todo sobre el mundo, preguntar como se hace#
    ...
    posible_xy_pacman = [PropSymbolExpr(pacman_str, x, y, time=t) for x,y in non_outer_wall_coords]
    posible_act_pacman = [PropSymbolExpr(direction, time=t) for direction in DIRECTIONS]

    pacphysics_sentences.append(exactlyOne(posible_xy_pacman))
    pacphysics_sentences.append(exactlyOne(posible_act_pacman))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x,y in non_wall_coords:
            posible_xy_pacman = posible_xy_pacman + [PropSymbolExpr(pacman_str, x, y, time=t)]
        
        KB.append(exactlyOne(posible_xy_pacman))
        ...
        for direction in actions:
            posible_act_pacman = posible_act_pacman + [PropSymbolExpr(direction, time=t)]

        KB.append(exactlyOne(posible_act_pacman))
        
        for x,y in non_wall_coords:
            successors = successors + [pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid)]

        KB.append(conjoin(successors))
...
def foodLogicPlan(problem) -> List:
    ...
    for x,y in food:
        food_points += [PropSymbolExpr(pacman_str, x, y, time=0)]
    ...
    for t in range(50):
        ...
        for x,y in non_wall_coords:
            successors_pacman = successors_pacman + [pacmanSuccessorAxiomSingle(x, y, t+1, walls)]
        
        KB.append(conjoin(successors_pacman))
...
def sentence4():
    ...
    # El Agente tiene flechas en t=1 si y solo si tiene flechas en t=0 y no dispara flecha en el t=0
    instance1 = agenteflecha_1 % (agenteflecha_0 & ~agentedispara_0)

    # El Agente dispare en t=0 y no tenga flecha en t=1 implica que el Wumpus está muerto en t=1.
    instance2 = (agentedispara_0 & ~agenteflecha_1) >> wumpusmuerto_1

    # Que el Wumpus no esté muerto en t=1, el Agente no dispare en t=0 y el Agente tenga flecha en t=0 implica que el Agente tiene flecha en t=1.
    instance3 = conjoin([~wumpusmuerto_1, ~agentedispara_0, agenteflecha_0]) >> agenteflecha_1

    # El Agente no dispara la flecha en t=0 y el Wumpus no está muerto en t=1 y el Agente tiene flecha en t=1.
    instance4 = conjoin([~agentedispara_0, ~wumpusmuerto_1, agenteflecha_1])

    return conjoin([instance1,instance2,instance3,instance4])

def sentence5():
    "*** MODIFICATION 2 ***"
    agente_0 = PropSymbolExpr("Agente",1,1,time=0)
    agente_1 = PropSymbolExpr("Agente",2,1,time=1)
    action1 = PropSymbolExpr("Norte",time=0)
    action2 = PropSymbolExpr("Este", time=0)

    # Agente en t= 1 en [2,1] y Agente en t=0 en [1,1] y se mueve al este en t=0 y no se mueve al norte en t = 0
    instance1 = agente_1 % conjoin([agente_0, action2, ~action1]) 
    instance2 = agente_1  # Agente en t= 1 en [2,1]
    instance3 = action2 | action1 # Agente en t=0 se mueve o al este o al norte en t=0
