...
def constructBayesNet(gameState):
    ...
    # Construyendo las aristas de la red bayesiana
    edges.append(('xPos', 'foodHouse'))
    edges.append(('yPos', 'foodHouse'))
    edges.append(('xPos', 'ghostHouse'))
    edges.append(('yPos', 'ghostHouse'))

    for v in obsVars:
        edges.append(('foodHouse', v))
        edges.append(('ghostHouse', v))

    # Construyendo la lista de variables de la red bayesiana
    variables = [X_POS_VAR, Y_POS_VAR] + HOUSE_VARS + obsVars

    # Construir el dominio de cada variable de la red bayesiana(posibilidades de cada variable)
    variableDomainsDict['foodHouse']  = HOUSE_VALS
    variableDomainsDict['ghostHouse'] = HOUSE_VALS
    variableDomainsDict['xPos'] = X_POS_VALS
    variableDomainsDict['yPos'] = Y_POS_VALS

    for v in obsVars:
        variableDomainsDict[v] = OBS_VALS


    # Tabla de probabilidad condicional vacía de la red bayesiana 
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    # Query => FoodHouse
    inference_factor = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidenceDict=evidence, eliminationOrder=eliminationOrder)
    ...
    for assignmentDict in inference_factor.getAllPossibleAssignmentDicts():
        ...
        # Obtenemos la posicion con mayor probabilidad
        if high_prob < prob:
            high_prob = prob
            position = assignmentDict
...
def constructInsuranceBayesNet():
    ...
    incomeFactor = bn.Factor([INCOME_VAR], [], net.variableDomainsDict())
    incomeFactor.setProbability({INCOME_VAR:HIGH_INCOME}, 0.3)
    incomeFactor.setProbability({INCOME_VAR:LOW_INCOME}, 0.7)

    housingFactor = bn.Factor([HOUSING_VAR], [], net.variableDomainsDict())
    housingFactor.setProbability({HOUSING_VAR:REAL_STATE}, 0.35)
    housingFactor.setProbability({HOUSING_VAR:TENANT}, 0.65)

    depositFactor = bn.Factor([DEPOSIT_VAR], [INCOME_VAR], net.variableDomainsDict())
    depositFactor.setProbability({INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT}, 0.1)
    depositFactor.setProbability({INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT}, 0.9)
    depositFactor.setProbability({INCOME_VAR: LOW_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT}, 0.6)
    depositFactor.setProbability({INCOME_VAR: LOW_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT}, 0.4)

    paymentFactor = bn.Factor([PAYMENT_VAR], [INCOME_VAR, DEPOSIT_VAR], net.variableDomainsDict())
    paymentFactor.setProbability({INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.05)
    paymentFactor.setProbability({INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.95)
    paymentFactor.setProbability({INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.5)
    paymentFactor.setProbability({INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.5)
    paymentFactor.setProbability({INCOME_VAR: LOW_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.45)
    paymentFactor.setProbability({INCOME_VAR: LOW_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.55)
    paymentFactor.setProbability({INCOME_VAR: LOW_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.6)
    paymentFactor.setProbability({INCOME_VAR: LOW_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.4)

    securityFactor = bn.Factor([SECURITY_VAR], [HOUSING_VAR, PAYMENT_VAR], net.variableDomainsDict())
    securityFactor.setProbability({PAYMENT_VAR:DEFAULT, HOUSING_VAR: REAL_STATE, SECURITY_VAR:SECURITY_GIVEN}, 0.01)
    securityFactor.setProbability({PAYMENT_VAR:DEFAULT, HOUSING_VAR: REAL_STATE, SECURITY_VAR:NO_SECURITY}, 0.99)
    securityFactor.setProbability({PAYMENT_VAR:DEFAULT, HOUSING_VAR: TENANT, SECURITY_VAR:SECURITY_GIVEN}, 0.5)
    securityFactor.setProbability({PAYMENT_VAR:DEFAULT, HOUSING_VAR: TENANT, SECURITY_VAR:NO_SECURITY}, 0.5)
    securityFactor.setProbability({PAYMENT_VAR:PAY_BACK, HOUSING_VAR: REAL_STATE, SECURITY_VAR:SECURITY_GIVEN}, 0.75)
    securityFactor.setProbability({PAYMENT_VAR:PAY_BACK, HOUSING_VAR: REAL_STATE, SECURITY_VAR:NO_SECURITY}, 0.25)
    securityFactor.setProbability({PAYMENT_VAR:PAY_BACK, HOUSING_VAR: TENANT, SECURITY_VAR:SECURITY_GIVEN}, 0.31)
    securityFactor.setProbability({PAYMENT_VAR:PAY_BACK, HOUSING_VAR: TENANT, SECURITY_VAR:NO_SECURITY}, 0.69)
    
    net.setCPT(INCOME_VAR, incomeFactor)
    net.setCPT(HOUSING_VAR, housingFactor)
    net.setCPT(DEPOSIT_VAR, depositFactor)
    net.setCPT(PAYMENT_VAR, paymentFactor)
    net.setCPT(SECURITY_VAR, securityFactor)

    return net

def inferenceOnInsuranceBayesNet(bayes_net):
    "*** MODIFICATION ***"
    queryVariables = [SECURITY_VAR]
    evidenceDict = {INCOME_VAR: HIGH_INCOME, HOUSING_VAR: TENANT}
