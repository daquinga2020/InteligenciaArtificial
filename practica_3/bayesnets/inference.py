...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        bayesNet:         The Bayes Net on which we are making a query.La Red Bayesiana sobre la que estamos realizando una consulta.
        queryVariables:   A list of the variables which are unconditioned
                          in the inference query.Una lista de las variables que no están condicionadas en la consulta de inferencia.
        evidenceDict:     An assignment dict {variable : value} for the
                          variables which are presented as evidence
                          (conditioned) in the inference query. 
        ...
        "*** YOUR CODE HERE ***"
        # Reducimos el tamaño de la tabla según la variable de evidencia
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Iteramos sobre las variables segun "eliminationOrder", realizando una union y eliminacion de la variable
        # Obtenemos la variable consulta y la variable de evidencia
        for elim_var in eliminationOrder:
            currentFactorsList, simplifiedFactor = joinFactorsByVariable(currentFactorsList, elim_var)
            
            # Eliminar el factor si tiene mas de una variable incondicionada 
            if len(simplifiedFactor.unconditionedVariables()) > 1:
                elim_factor = eliminate(simplifiedFactor, elim_var)
                currentFactorsList.append(elim_factor)
        
        # Unir y normalizar el factor para que sus probabilidades sumen 1
        simplifiedFactor = joinFactors(currentFactorsList)
        normalizeFactor = normalize(simplifiedFactor)
